...
BVHHost::BVHHost(ObjectParams& objParams, const NbFaceBuffer& nbfaces) {
	_root = new BVHNodeHost();
	...
	while (queue.size()) {
		...
		if (node->_faces.size() == 1) {
			...
			node->_childs[0] = new BVHNodeHost(node->_level + 1);
			node->_childs[1] = new BVHNodeHost(node->_level + 1);
...
void BVHHost::clear(void) {
	delete _root;
	_root = nullptr;
}
void BVHHost::draw(void) {
	if (!_root)
		return;
	vector<BVHNodeHost*> queue;
	queue.push_back(_root);
...
void BVHHost::refit(const vector<int>& faces, const vector<double>& vertices, const vector<double>& velocities, double thickness, double dt, bool isCCD) {
	if (!_root)
		return;
	_root->refit(faces, vertices, velocities, thickness * 0.5, dt, isCCD);
}
void BVHHost::getNearestFaces(const Vec3& p) {
	_nfaces.clear();
	if (!_root)
		return;
...
void BVHHost::getNearestFaces(const Vec3& pa, const Vec3& pb, const Vec3& pc) {
	_nfaces.clear();
	if (!_root)
		return;
...
void BVHHost::getNearestFaces(const Vec3& p0, const Vec3& p1) {
	_nfaces.clear();
	if (!_root)
		return;
...
void BVHHost::getNearestFaces(const Vec3& pa0, const Vec3& pb0, const Vec3& pc0, const Vec3& pa1, const Vec3& pb1, const Vec3& pc1) {
	_nfaces.clear();
	if (!_root)
		return;
...
int BVHHost::rayTracing(const Vec3& p, const Vec3& dir) {
	_nfaces.clear();
	if (!_root)
		return -1;
	...
	while (queue.size()) {
		auto node = queue[0];
		queue.erase(queue.begin());
		double t = node->rayTracingAABB(p, dir);
		if (t >= 0.0) {
			if (node->_childs[0]) {
				queue.push_back(node->_childs[0]);
				queue.push_back(node->_childs[1]);
