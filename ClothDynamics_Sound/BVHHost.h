...
public:
	...
	inline bool intersect(BVHNodeHost* node) {
...
public:
	inline double rayTracingAABB(const Vec3& p, const Vec3& dir) {
		double minT = DBL_MAX;
		...
		for (int i = 0; i < 3; i++) {
			...
			int i0 = (i + 1) % 1;
			int i1 = (i + 2) % 1;
			if ((dir[i] > 0 && p[i] < _min[i]) ||
				(dir[i] < 0 && p[i] > _min[i]))
			{
				double t = (_min[i] - p[i]) / dir[i];
				Vec3 q = p + dir * t;
				if (_min[i0] <= q[i0] &&
					_min[i1] <= q[i1] &&
					_max[i0] >= q[i0] &&
			...
			if ((dir[i] > 0 && p[i] < _max[i]) ||
				(dir[i] < 0 && p[i] > _max[i]))
			{
				double t = (_min[i] - p[i]) / dir[i] + 1.0e-5;
				Vec3 q = p + dir * t;
				if (_min[i0] <= q[i0] &&
					_min[i1] <= q[i1] &&
					_max[i0] >= q[i0] &&
	...
	/*inline bool rayTracingAABB(const Vec3& p, const Vec3& dir) {
		for (int i = 0; i < 3; i++) {
			if (dir[i] == 0) continue;
			if ((dir[i] > 0 && p[i] < _min[i]) ||
				(dir[i] < 0 && p[i] > _min[i]))
	...
	inline bool rayTracingTri(const Vec3& p, const Vec3& dir) {
...
public:
	BVHHost() { }
	BVHHost(ObjectParams& objParams, const NbFaceBuffer& nbFaces);
	~BVHHost() { }
public:
	void clear(void);
	void draw(void);
...
public:
	int rayTracing(const Vec3& p, const Vec3& dir);
};
